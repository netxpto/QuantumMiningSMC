...
public:
	...
	void setFilterType_PSclk(pulse_shapper_filter_type fType) { PulseShaper_1.setFilterType(fType); };
	pulse_shapper_filter_type const getFilterType_PSclk(void) { return PulseShaper_1.getFilterType(); };

	void setPulseWidthDetection(double pWidth) { PulseShaper_1.setPulseWidth(pWidth); };
	double const getPulseWidthDetection() { return PulseShaper_1.getPulseWidth(); };
...
private:
	...
	MQamMapper MQamMapper_{ { &BS_out_1 },{ &MQM_out} };
	...
	PhotonStreamXY SP_in{ "SP_in.sgn" }, SPPBS_in{ "SPPBS_in.sgn" };
	PolarizationRotator PolarizationRotator_{ {&SP_in, &PS_out}, {&SPPBS_in} };
	...
	SinglePhotonDetector SinglePhotonDetector_0{ {&SPPBS_out},{&SPD0_out} };
	SinglePhotonDetector SinglePhotonDetector_1{ {&SPPBS_out},{&SPD1_out} };

	TimeContinuousAmplitudeContinuousReal CLK_cd_in{ "CLK_cd_in.sgn" };
	Clock Clock_{ {},{&CLK_cd_in} };

	TimeContinuousAmplitudeContinuousReal PS_clk_out{ "PS_clk_out.sgn" };
	PulseShaper PulseShaper_1{ {&CLK_cd_in},{&PS_clk_out} };
