...
public:
	...
	// Signal constructors
	Signal() {};
	Signal(string fName) { setFileName(fName); setSaveSignal(true); };
	Signal(string fileName, string folderName) { setFileName(fileName); setFolderName(folderName); setSaveSignal(true); };
	Signal(int bLength) { setBufferLength(bLength); };
	Signal(string fName, bool sSignal) { setFileName(fName); setSaveSignal(sSignal); };
	Signal(string fileName, bool sSignal, int bLength) { setFileName(fileName); setSaveSignal(sSignal); setBufferLength(bLength); };
	Signal(string fileName, string folderName, bool sSignal) { setFileName(fileName); setFolderName(folderName); setSaveSignal(sSignal); };
	Signal(string fileName, string folderName, bool sSignal, int bLength) { setFileName(fileName); setFolderName(folderName); setSaveSignal(sSignal); setBufferLength(bLength);	};
...
public:
	...
	void setNumberOfInputSignals(std::size_t nOfInputSignal) { numberOfInputSignals = nOfInputSignal; };
	std::size_t getNumberOfInputSignals() { return numberOfInputSignals; };

	void setNumberOfOutputSignals(std::size_t nOfOutputSignal) { numberOfOutputSignals = nOfOutputSignal; };
	std::size_t getNumberOfOutputSignals() { return numberOfOutputSignals; };

	vector<Signal *> inputSignals;
	vector<Signal *> outputSignals;

	
	std::size_t numberOfInputSignals;	// passar para private, 2018/07/15
	std::size_t numberOfOutputSignals;  // passar para private, 2018/07/15
