...
void FIR_Filter::initializeFIR_Filter(void) {
	...
	if (!getSeeBeginningOfImpulseResponse()) {
		int aux = (int)(((double)impulseResponseLength) / 2);
		int previousFirstValue = inputSignals[0]->getFirstValueToBeSaved();
		outputSignals[0]->setFirstValueToBeSaved(aux+previousFirstValue);
	}
