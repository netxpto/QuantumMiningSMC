#include "homodyne_receiver_withoutLO.h"
...
//
//	B04 and B05 defined and initialized in h file. TI_Amplifier is a SuperBlock
//	and apparently does not work with initializeBlock. Currently there is no
//	initializeSuperBlock method. So it is called on declaration, as I'm currently
//	don't have time to look for another way, and this one apparently works.

	B06.initializeBlock(vector<Signal*> { }, vector<Signal*> { &HMD09 }); //Noise
	B07.initializeBlock(vector<Signal*> { }, vector<Signal*> { &HMD10 }); //Noise
	
	B08.initializeBlock(vector<Signal*> { &HMD07, &HMD09}, vector<Signal*> { &HMD11 }); //Add
	B09.initializeBlock(vector<Signal*> { &HMD08, &HMD10 }, vector<Signal*> { &HMD12 }); //Add

	B10.initializeBlock(vector<Signal*> { &HMD11}, vector<Signal*> { &HMD13 }); //Pulse Shaper
