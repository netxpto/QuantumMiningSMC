...
int main() {
	...
	double BTs{ 0.25 };
	int impulseResponseTimeLength{ 64 };
	int transerFunctionLength{ 1024 };
	...
	PulseShaperFd20181110 B6{ vector<Signal*> { &S4 }, vector<Signal*> { &S6 } };
	B6.setRollOffFactor( rollOffFactor);
	B6.setTransferFunctionLength(transerFunctionLength);
	B6.setSeeBeginningOfTransferFunction(false);
	B6.setFilterType(RaisedCosineTF);
	B6.setFilterPhaseTypeTf("LinearPhase");
	...
	B7.setFilterPhaseTypeIr("LinearPhase");
