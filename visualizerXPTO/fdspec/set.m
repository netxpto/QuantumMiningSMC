...
while ind < length(varargin)
    ...
    elseif iscell(varargin{ind})  % cell array of properties
        ...
        if length(fields)~=length(vals)
            error(message('signal:fdspec:set:InvalidDimensions'))
        end
        ind = ind + 2;
    else % property name
        fields = varargin(ind);
...
%                     end
        ...
        case {'callback', 'radiogroup'}
            if ~isstr(vals{i})
                error(message('signal:fdspec:set:MustBeString', fields{ i }));
            end
            objud = setfield(objud,fields{i},vals{i});
        case 'integer'
            if ~isequal(vals{i},0) & ~isequal(vals{i},1)
                error(message('signal:fdspec:set:SigErr'))
            end
            objud = setfield(objud,fields{i},vals{i});
        case {'editable','visible'}
            if ~isequal(vals{i},'on') & ~isequal(vals{i},'off')
                error(message('signal:fdspec:set:MustBeOnOff', fields{ i }, '''on''', '''off'''))
            end
            objud = setfield(objud,fields{i},vals{i});
