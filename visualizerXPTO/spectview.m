...
elseif ~ischar(varargin{1})
    ...
    %% Label list of PSD methods available
    set(findobj('Tag', 'methodPopup'), 'String', {'Burg', 'Covariance', 'FFT', 'Mod. Covar.', 'MTM', 'MUSIC', 'Welch', 'Yule AR', '2-sided Welch'});
    
    %% Add new method internally
    fig = gcf;
    ud = get(fig, 'UserData');
    ud.methods(9).methodName = '2-sided Welch';
    ud.methods(9).methodHelp = {'2-sided Welch Method'; ' '; 'This method estimates the Power Spectral Density (PSD)'; 'using Welch overlapped segment averaging estimator.'};
    ud.methods(9).label = [];
    ud.methods(9).type = [];
    ud.methods(9).default = [];
    ud.methods(9).subordinates = [];
    ud.methods(9).confidenceFlag = 0;
    ud.methods(9).popupString = [];
    ud.methods(9).helpString = [];
    ud.methods(9).computeFcn = [];
    set(gcf, 'UserData', ud);
    %%
...
switch varargin{1}
    ...
    case 'linedown'
        ...
        if ~isempty(ind)
            ...
            choice = '2-sided welch';  % (see spinit.m)
...
%         methodNameList = get(ud.hand.methodPopup,'string');
    ...
    case 'apply'
        ...
        if length(varargin) <= 1
            ...
            spect = spectra(varargin{2});
            spect.confid.enable = 0;
            spect.confid.level = '.95';
            spect.confid.Pc = [];
            %spect.specs.valueArrays = {{0x1 cell}};
            spect.specs.valueArrays = [];
            spect.specs.methodName = {'2-sided Welch'};
            %spect.specs.methodNum = 1;
            ud.focusIndex = find(activeSpectra == varargin{2});
        end
        %%
...
%errstr = ''; % The value assigned to variable 'errstr' might be unused
...
if ~strcmp(spect.signalLabel, '') % IF Signal
    sigStruc = sptool('Signals');
    sigInd = findcstr({sigStruc.label}, spect.signalLabel); % Signal index
    sigdata = sigStruc(sigInd).data; % Signal data (temporal)
else % IF not Signal (Filter)
    filterStruc = sptool('Filters');
    filterLabel = strjoin(strsplit(spect.label, 'spect_'), ''); % Filter label
    filterInd = findcstr({filterStruc.label}, filterLabel); % Filter index
    filterdata = filterStruc(filterInd).imp; % Filter data (temporal)
end
...
if spect.confid.enable && methods(ToolMethodNum).confidenceFlag
    ...
    if ~strcmp(spect.signalLabel, '') % IF Signal
        ...
        [errstr,spect.P,spect.f,spect.confid.Pc] = ...
        feval(methods(ToolMethodNum).computeFcn,filterdata,spect.Fs,...
        valueArray,confidenceLevel);
    end
else
    if ToolMethodNum <= 8    
    ...
    %% Pwelch Unilateral Spectrum    
        % Error String
        errstr = '';
        % Power Spectral Density (spect.P) and Frequency (spect.f) vectors
        if ~strcmp(spect.signalLabel, '') % IF Signal
            x = sigdata;
        else % IF not Signal (Filter)
            x = filterdata;
        end
        Nwind = 256; % Default segment/window length
        Overlap = 0; % Number of overlapped samples
        Nfft = 1024; % Default FFT/DFT length
        fs = spect.Fs; % Sampling frequency
        if length(x) <= Nfft
            Nfft = length(x);
            if Nfft >= 16
                Nwind = round(Nfft/4);
        ...
        [pxx, f] = pwelch(x, Nwind, Overlap, Nfft, fs);
        spect.P = pxx; 
        spect.P = spect.P/2; % For bilateral spectra
        spect.f = f; 
    %%    
    end 
end
...
if numel(ud.SPToolIndices)>1,
    ...
    switch lower(cm)
        ...
        case '2-sided welch' % New method
            sts = 'Power Spectral Density';
