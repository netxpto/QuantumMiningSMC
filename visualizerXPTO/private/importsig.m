...
case 'fields'
  popupString = getString(message('signal:sptoolgui:Signal'));
  flds.form = 'Signal';
  flds.fields = {getString(message('signal:sptoolgui:Data'))};
  FsFlag = 1;
  defaultLabel = 'sig';
...
%  since the fields and their order have to be the same for every object!
    ...
    if ndims(struc.data)~=2 | ~strcmp(class(struc.data),'double')
       errstr = getString(message('signal:sptoolgui:TheDataForTheSignalMustBeA2DMatrixOfClassdouble'));
    end
    if ~all(size(struc.Fs)==1) | ~isreal(struc.Fs) | struc.Fs<=0
        errstr = getString(message('signal:sptoolgui:TheSamplingFrequencyMustBeAPositiveRealScalar'));
    end
...
case 'merge'
    ...
    if isempty(struct2.lineinfo)
        ...
        if ~isempty(newstruct.lineinfo) && ...
            (~isfield(newstruct.lineinfo,'columns') || ...
