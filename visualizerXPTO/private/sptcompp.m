...
components(1).defaultClient = 'sigbrowser.sigbrowseAdapter';
components(1).multipleSelection = 1;
components(1).importFcn = 'importsig';
components(1).types = {'vector' {} 
                       'array' {'fontweight','bold'} };
components(1).verbs.owningClient = 'sigbrowser.sigbrowseAdapter';
components(1).verbs.buttonLabel = getString(message('signal:sptoolgui:View'));
...
components(2).verbs(1).buttonLabel = getString(message('signal:sptoolgui:View'));
components(2).verbs(1).action = 'view';
components(2).verbs(2).owningClient = 'filtdes';
components(2).verbs(2).buttonLabel = getString(message('signal:sptoolgui:New'));
components(2).verbs(2).action = 'create';
components(2).verbs(3).owningClient = 'filtdes';
components(2).verbs(3).buttonLabel = getString(message('signal:sptoolgui:Edit'));
components(2).verbs(3).action = 'change';
components(2).verbs(4).owningClient = 'applyfilt';
components(2).verbs(4).buttonLabel = getString(message('signal:sptoolgui:Apply'));
...
components(3).verbs(1).buttonLabel = getString(message('signal:sptoolgui:View'));
components(3).verbs(1).action = 'view';
components(3).verbs(2).owningClient = 'spectview';
components(3).verbs(2).buttonLabel = getString(message('signal:sptoolgui:Create'));
components(3).verbs(2).action = 'create';
components(3).verbs(3).owningClient = 'spectview';
components(3).verbs(3).buttonLabel = getString(message('signal:sptoolgui:Update'));
